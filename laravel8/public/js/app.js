...
window.riseIcon = function (event, kind, v) {
  getFetch('render_icon/' + (v ? 'un' : '') + kind).then(function (res) {
    ...
    icon.style.scale = 3;
    icon.innerHTML = res.html; // On positionne l'icône à l'endroit du clic

    icon.style.left = event.clientX + window.scrollX + 'px';
    icon.style.top = event.clientY + window.scrollY + 'px';
    document.getElementById('top-page').appendChild(icon); // On upprimee l'icône après l'animation
