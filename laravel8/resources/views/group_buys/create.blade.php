...
<div class="min-w-full max-w-xs">
    <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" action="{{ route('group_buys.store') }}" method="POST">
        ...
        <input type="hidden" id="max_product_nb" name="max_product_nb" value="{{ old('max_product_nb', 0) }}"/>
        @error('max_product_nb')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        <div id="all_products_bought" class="flex flex-col gap-4 my-4">
            @for($nb = 0; $nb < old('max_product_nb', 0); $nb++)
        ...
        <div class="flex items-center justify-between">
            ...
            <x-form.cancel href="{{ route('my_products') }}"/>
        </div>
    </form>
    </div>
@endsection