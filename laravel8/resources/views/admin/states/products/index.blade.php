    ...
    <div id="dashboard_table">
        ...
        <div class="right">
            ...
            <div class="relative flex justify-center border-b-2 mb-4">
                <span class="absolute left-0 font-semibold">{{ isset($product_states->links) ? $product_states->links()->paginator->total() : count($product_states) }} Resultats</span>
                ...
                @foreach($product_states as $product_state)
                    <div class="item_list flex flex-between gap-4 p-2 shadow rounded border-l-4 border-indigo-400 hover:shadow-lg transform hover:scale-105">
                        <div class="flex w-full items-center gap-8">
                            <span class="font-bold"># {{ $product_state->id }}</span>
                            <x-utils.v_line />
                            <span>{{ $product_state->label }}</span>
                        </div>
                        <div class="flex flex-around items-center gap-4 text-sm">
                            <a class="bg-indigo-600 text-gray-200 rounded p-1 px-4 hover:bg-indigo-400 hover:text-white" href="{{ route('states.products.edit', $product_state->id) }}">Editer</a>
                            <form action="{{ route('states.products.destroy', $product_state->id) }}" method="post">
                                @csrf
                                @method('DELETE')
