<x-app-layout>
    <div id="dashboard_table">
        ...
        <div class="right">
            ...
            <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" action="{{ route('vg_developers.update', $vg_developer) }}" method="POST">
                @csrf
                @method('put')
                ...
                <div class="w-full flex inline-flex justify-between mb-4 gap-4">
                    <div class="w-3/4">
                        ...
                        <x-form.input name="label" placeholder="Naughty Dog" value="{{ old('label', $vg_developer->label) }}"/>
                    </div>
                    <div class="w-1/4">
                        <x-form.label for="year_created" block required>Année de création</x-form.label>
                        <x-form.input name="year_created" placeholder="2000" value="{{ old('year_created', $vg_developer->year_created) }}"/>
                    </div>
                </div>
                <div class="my-4">
                    <x-form.label for="description" block>Description</x-form.label>
                    <x-form.textarea name="description">{{ old('description', $vg_developer->description) }}</x-form.textarea>
                </div>
