<div id="product_picture" class="flex {{ (count($photos) > 1)? 'justify-between' : 'justify-center' }} gap-2">
    <div id="picture_displayed" onMouseOver="onPicture(true);" onMouseOut="onPicture(false);" onClick="toggleZoomPictures(1);" class="relative {{ (count($photos) > 1)? 'flex w-4/5' : '' }}">
        ...
        <img id="big_picture" class="cursor-pointer object-contain" src="{{ asset($dir.$photos->first()->label) }}"/>
    </div>
    @if(count($photos) > 1)
        <div class="flex flex-col w-1/5 gap-2 overflow-x-hidden overflow-y-auto overscroll-contain scrollbar-thin scrollbar-thumb-red-500 scrollbar-track-red-200 scrollbar-thumb-rounded pr-3" style="height:450px;">
