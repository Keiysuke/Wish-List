...
<div class="flex gap-4 mb-4 items-center">
    <div class="w-2/5">
        <div class="relative flex inline-flex justify-between w-full">
            ...
            <x-svg.search class="icon-xs cursor-pointer" onClick="webSearch('{{ 'label'.$nb }}')"/>
        </div>
    ...
    <div class="w-2/5">
        ...
        <select name="developer_id{{ $nb }}" id="developer-id{{ $nb }}" class="pl-2 h-10 block w-full rounded-md bg-gray-100 border-transparent">
            <option value="to_create">A créer</option>
            @foreach($developers as $developer)
                <option value="{{ $developer->id }}" @if(old('developer_id'.$nb) == $developer->id) selected @endif>{{ $developer->label }}</option>
            @endforeach
        </select>
        @error('developer_id')
