...
<select name="product_bought_purchase_id_{{ $nb }}" id="product_bought_purchase_id_{{ $nb }}" class="dynamic_select_data pl-2 h-10 block w-full rounded-md bg-gray-100 border-transparent">
    @foreach($purchases as $purchase)
        <option class="relative" id="product_bought_purchase_id_{{ $nb }}_{{ $purchase->id }}" value="{{ $purchase->id }}" @if(old('product_bought_purchase_id_'.$nb, ($edit_purchase->id ?? null)) == $purchase->id) selected @endif>
