...
<x-notif :notif="$notif" icon="svg.clock" kind="warning" title="{{ $expire ? __('Soon available') : __('Soon expired') }}">
    <a href="{{ route('products.showFromNotification', ['product' => $notif->data['product_id'], 'notification' => $notif->id]) }}" class="flex items-center mx-1 bg-white" title="{{ $notif->data['product_label'] }}">
        ...
        <span class="w-4/6 flex justify-center text-black text-sm">{{ $expire ? __('Disponible') : __('Expire') }} dans {{ $notif->data['days'] }} jour(s)</span>
    </a>
</x-notif>