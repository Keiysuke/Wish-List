@php($crowdfunding = $purchase->crowdfunding())
...
<div class="{{ $class }} h-full w-full" onClick="toggle_thumbnail({{ $cpt }})">
    <div class="relative bg-{{ is_null($crowdfunding) ? 'orange' : 'yellow' }}-500 font-semibold uppercase rounded rounded-b-none text-white p-2">
        <div class="absolute left-2 top-3">
            <x-svg.cart class="w-5 h-5"/>
        </div>
        {{ is_null($crowdfunding) ? 'Acheté' : 'Soutenu' }}
    ...
    <div class="flex flex-col justify-around h-40 py-4">
        <p><span class="font-semibold">
            @if(is_null($crowdfunding))
                Etat :</span> {{ $purchase->productState->label }}
            @else
                Avancée :</span> {{ $crowdfunding->state->label }}
            @endif
        </p>
        ...
        @endif
        @php($url = is_null($crowdfunding) ? $purchase->website->url : $crowdfunding->project_url)
        <p class="text-sm text-gray-500">via <a class="link no-propagate" href="{{ $url }}" target="_blank">{{ $purchase->website->label }}</a>{{ ', le '.$purchase->date() }}</p>
    </div>
